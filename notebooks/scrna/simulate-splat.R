...
for (batch_scale in batch_scales) {
  ...
  ax <- plotPCA(sce, colour_by = "Batch", shape_by = "Group")
  file <- sprintf("tmp/fig/splat/pca-bal_%.2f-01.jpg", batch_scale)
  ggsave(file, ax)
}


# Investigate saved data set
...
#
# TODO: Data set simulation: Are that batch effects actually varying alot?
file1 <- sprintf(
  "data/simulated/scrna/loc0/splat-scale_%.2f-%02d.rds",
  batch_scale, i
)
# TODO: RVP first (red in colour), leave LISI last (different axis)?
